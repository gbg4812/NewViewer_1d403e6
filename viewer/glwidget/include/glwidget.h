...
class GLWidget : public QOpenGLWidget, public QOpenGLFunctions_3_3_Core {
  ...
  public slots:
    /*!
    Opens a file dialog enabling the user to select one or more OBJ files to add
    to the scene. All loaded plugins get their onObjectAdd() method invoked for
    each loaded object.
    */
    ...
    */
    PLUGINGLWIDGET_EXPORT void drawPoint(const Vector &pos, float r, float g,
                                         float b);

    /*!
    Returns the bounding box of the scene, enlarged to include also a
