...
GLWidget::GLWidget(const string &args)
    ...
    if (gradepluginpos != string::npos) {
        ...
        gradepluginpos +=
            args.substr(gradepluginpos + 15).find_first_not_of(" ");
...
void GLWidget::setPluginPath(const QString &p) { pluginPath = p; }

const QString &GLWidget::getPluginPath() const { return pluginPath; }

void GLWidget::initializeGL() {
    // glewExperimental = GL_TRUE;
    // glewInit(); // necessari per cridar funcions de les darreres versions
    // d'OpenGL
    ...
    vs.compileSourceCode(
        "#version 330 core \n layout (location = 0) in vec3 vertex; layout "
        "(location = 1) in vec3 normal; uniform mat4 "
        "modelViewProjectionMatrix; out vec4 frontColor;    void main() { "
        "frontColor = vec4(abs(normal),0.25); gl_Position = "
        "modelViewProjectionMatrix * vec4(vertex, 1.0); }");
    QOpenGLShader fs(QOpenGLShader::Fragment);
    fs.compileSourceCode("#version 330 core \n in vec4 frontColor; out vec4 "
                         "FragColor; void main() {FragColor = frontColor;}");
...
QOpenGLShaderProgram *GLWidget::defaultProgram() { return &unlitProgram; }
...
void GLWidget::resetPluginsToDefaults() {
    for (unsigned int i = 0; i < plugins.size(); ++i) {
        std::cout << "Unloading " << plugins[i]->fileName().toStdString()
                  << std::endl;
        plugins[i]->unload();
        delete plugins[i];
    }
...
void GLWidget::unloadPlugin(int i) {
    if (i < 0 or unsigned(i) >= plugins.size())
        return;
    std::cout << "Unloading " << plugins[i]->fileName().toStdString()
              << std::endl;
    plugins[i]->unload();
...
Scene *GLWidget::scene() { return &pscene; }

Camera *GLWidget::camera() { return &pcamera; }
...
void GLWidget::drawAxes() {
    ...
    if (!created) {
        ...
        QOpenGLShader vs(QOpenGLShader::Vertex);
        vs.compileSourceCode(
            "#version 330 core \n in vec3 vertex; in vec3 color; uniform mat4 "
            "modelViewProjectionMatrix; out vec4 frontColor;    void main() { "
            "frontColor = vec4(color,1); gl_Position = "
            "modelViewProjectionMatrix * vec4(vertex, 1.0); }");

        // FS
        QOpenGLShader fs(QOpenGLShader::Fragment);
        fs.compileSourceCode(
            "#version 330 core \n in vec4 frontColor; out vec4 FragColor; void "
            "main() {FragColor = frontColor;}");
        ...
        float coords[] = {0, 0, 0, L, 0, 0, 0, 0, 0, 0, L, 0, 0, 0, 0, 0, 0, L};
    ...
    scale.scale(r, r, r);
    QMatrix4x4 MVP =
        camera()->projectionMatrix() * camera()->viewMatrix() * scale;
...
void GLWidget::paintGL(void) {
    ...
    for (int i = int(plugins.size()) - 1; i >= 0; --i) // notice reverse order
        if (qobject_cast<Plugin *>(plugins[i]->instance())->paintGL())
            break;
...
void GLWidget::executeTest() {
    ...
    if (!QFile::exists(testFilename)) {
        ...
        if (!QFile::exists(testFilename)) {
            cerr << "Cannot find test file:" << (testFilename).toStdString()
                 << endl;
            return;
        }
    }
    ...
    if (!f.is_open()) {
        cerr << "Cannot open test file " << (testFilename).toStdString()
             << endl;
    ...
    while (!f.eof()) {
        ...
        if (cmd == "clearScene") {
            ...
            if (!QFile::exists(QString(s.c_str()))) {
                ...
                s = (models + QString("/") + QFileInfo(s.c_str()).fileName())
                        .toStdString();
...
#ifdef __Apple__
            if (img.size() !=
                size()) { // seems we're in a machine w. retina display
                img = img.scaled(size(), Qt::KeepAspectRatio,
                                 Qt::SmoothTransformation);
            }
#endif
            QString name = pluginToGrade.c_str() + QString("-out") + s.c_str() +
                           QString(".png");
            img.save(name);
        } else if (cmd == "exit") {
            QApplication::quit();
...
void GLWidget::loadPlugin() {
    QStringList list = QFileDialog::getOpenFileNames(
        NULL, "Select one or more plugins to open", pluginPath,
...
int GLWidget::loadPlugins(const QStringList &list) {
    ...
    // make sure all plugins know about the latest plugin that overrides
    // drawScene
    for (unsigned int i = 0; i < plugins.size(); ++i) {
...
void GLWidget::addObjectFromFile(const QString &filename) {
    ...
    cout << "Added object: " << fi.fileName().toStdString() << " (" << name
         << ")" << endl;
}
...
void GLWidget::addObject() {
    ...
    QStringList files = QFileDialog::getOpenFileNames(
        NULL, "Select one or more models to open", models, "Models (*.obj)");
