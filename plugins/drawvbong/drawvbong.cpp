...
DrawVBOng::~DrawVBOng() { cleanUp(); }

void DrawVBOng::onSceneClear() { cleanUp(); }
...
void DrawVBOng::onObjectAdd() { addVBO(scene()->objects().size() - 1); }
...
void DrawVBOng::addVBO(unsigned int currentObject) {
    ...
    for (auto &&f : obj.faces()) {
        ...
        colors.push_back(fabs(V.x()));
        colors.push_back(fabs(V.y()));
        colors.push_back(fabs(V.z()));
    ...
    g.glBufferData(GL_ARRAY_BUFFER, sizeof(float) * colors.size(), &colors[0],
                   GL_STATIC_DRAW);
    g.glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, 0, 0);
    g.glEnableVertexAttribArray(2);

    g.glBindBuffer(GL_ARRAY_BUFFER, stBufferID);
    g.glBufferData(GL_ARRAY_BUFFER, sizeof(float) * texCoords.size(),
